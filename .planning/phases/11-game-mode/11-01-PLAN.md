...
1. Update `src/data/history.ts` to add getTodaysPuzzle() (synchronous since data is embedded):
   ```typescript
   import { findByDate } from './wordleAnswers';
...
2. Update main.ts initialization:
   ...
   // Game state - currentPuzzle already exists in main.ts
   let gameMode: boolean = true; // Default to game mode

   // On app start, load today's puzzle (synchronous)
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Complete Wordle game mode transformation with auto-color reveal</what-built>
  <how-to-verify>
    1. Run: npm run dev
    2. Visit: http://localhost:5173
    3. Verify header shows "Wordle #XXX" (the puzzle number)
    4. Type a 5-letter word (e.g., "crane") and press Enter
    5. Confirm: Colors reveal automatically with flip animation
    6. Confirm: You cannot click cells to change colors (game mode)
    7. Confirm: Keyboard hints update with revealed colors
    8. Confirm: Suggestions filter based on revealed feedback
    9. Play until win or loss:
       - Win: Shows success message
       - Loss (6 guesses): Shows answer reveal
    10. Click Practice button, select a past puzzle:
       - Verify same auto-reveal behavior works
       - Verify stats do NOT update for practice games
    11. Click Reset Game: Verify game restarts with same puzzle
  </how-to-verify>
